...
export async function PATCH(request: Request) {
  ...
  let body: Partial<ResumeTemplateInsert> & { id?: string };
  try {
    body = (await request.json()) as Partial<ResumeTemplateInsert> & { id?: string };
  } catch {
    return NextResponse.json({ error: "Request body must be valid JSON." }, { status: 400 });
  }
  ...
  const update: ResumeTemplateInsert = {
    name: body.name,
    content: body.content,
    notes: body.notes ?? null,
  };

  const { data, error } = await supabase.from("resume_templates").update(update).eq("id", body.id).select("*").single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  return NextResponse.json({ data });
}
