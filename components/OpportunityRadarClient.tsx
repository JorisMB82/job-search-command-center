...
export function OpportunityRadarClient() {
  ...
  async function seedSources() {
    for (const source of starterSources) {
      if (!sources.some((item) => item.url === source.url)) await createSource(source);
    }
    await loadAll();
  }
