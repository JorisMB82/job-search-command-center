...
export function OpportunityRadarClient() {
  ...
  async function createSource(source: SourceDraft = newSource) {
    const response = await fetch("/api/radar/sources", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(sourcePayload(source)) });
    const payload = await response.json();
  ...
  async function seedSources() { for (const source of starterSources) await createSource(source); await loadAll(); }
